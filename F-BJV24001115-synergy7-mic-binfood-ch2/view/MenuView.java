...
import util.PromoUtils;

import static util.AdditionalUtils.*;
...
public class MenuView {
    ...
    public static void mainMenu() {
        ...
        for (int i = 0; i<getMenu().size(); i++) {
            MenuItem menuItem = getMenu().get(i);
            System.out.printf(
                    "%d. %-13s | %16s | %16s | %16s %n",
                    (i+1),
                    menuItem.getName(),
    ...
    public static void receiptMenu() {
        try {
            int order = 1;
            File receiptFile = new File(String.format("receipt(%s-%04d).txt", getCurrentTime("yyyy-MMdd"), order));
            while (receiptFile.exists()) {
                order++;
                receiptFile = new File(String.format("receipt(%s-%04d).txt", getCurrentTime("yyyy-MMdd"), order));
            }
